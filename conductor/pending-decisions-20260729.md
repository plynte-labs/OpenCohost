...
| 2.4 | **Is the legacy profile list still a contract?** `tests/test_profile_tone.py:129` asserts `Hagg` and `Akira (Uncensored)`, which exist in **neither** your `perfiles.json` **nor** the shipped `default_profiles.json`. The test fails everywhere, including CI, and reads your gitignored user state as its primary source | `tracks/residual_cleanup_20260729/` R5 | Drop the two dead names and switch the test to read `default_profiles.json` only — a contract test must not depend on your mutable state |
