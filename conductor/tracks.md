...
- [~] **Track: RAM / LLM Hardening — VRAM-honest core for the RTX 3060 12GB**
  ...
  *Status 2026-06-29: Config-hardening phase DONE (branch feat/ollama-config-hardening-20260629). 8-agent audit "Ollama vs bare llama.cpp" → STAY on Ollama (same llama.cpp engine; the wins are config, not a different backend). Set OLLAMA_FLASH_ATTENTION=1 / OLLAMA_KV_CACHE_TYPE=q8_0 / OLLAMA_GPU_OVERHEAD=1GiB via setdefault at ollama_startup (strict-TDD, 11 passed). Targets the gemma4:e4b (9.6GB) spill risk on the 12GB display-sharing card. Decision + how-to in docs/adr/ADR-022 (backend decision, with diagrams) + ADR-023 (config hardening). OWNER OWED: update Ollama 0.30.7→0.30.11, set the 3 vars as SYSTEM env vars (the setdefault only reaches the app-launched daemon), verify gemma4:e4b shows 100% GPU + flash-attn active via `ollama ps`. Engram: decision/audit-stay-on-ollama-do-not-migrate-to-bare-llama-cpp.*
