...
def test_memoria_list_shape_includes_title_but_not_content(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        # memoria_draft_visibility_20260725: neither insert above specifies
        # `status`, so the table's DEFAULT 'draft' applies to both rows —
        # draft=True should round-trip the same way imported does.
        for item in body["items"]:
